...
长期上，App 中的 CMS 不应只是普通表单，而应成为“意图驱动的 prompt 预处理器”：

- 用户只触发高层动作，例如 `发布`
- 系统自动判断 create / update / delete
- 系统自动识别 Capsule / Issue
- 系统自动拼装内置 prompt 与 Copilot / AI 协作
...
- 不把关键逻辑写死在某个页面组件里
- 不把 prompt 逻辑散落在用户输入里，而应沉淀为系统内置流程
...
当前迭代暂不推进这些平台化事项；它们只作为远期背景，避免短期设计与长期方向明显冲突。
